...
## Описание проекта
Необходимо подготовить прототип модели машинного обучения для ее использования в золотодобывающей отрасли. Модель должна предсказать коэффициент восстановления золота из золотосодержащей руды Имеются данные с параметрами добычи и очистки.
...
В ходе исследования была обработана данные по добыче золота с параметрами на разных этапах обработки. Данные были проверены путем вычисления эффективности обогащения и подготовлены для анализа. В результате исследования удалось получить вывод о том, что концентрация Золота с каждым этапом очистки постепенно увеличивается.
